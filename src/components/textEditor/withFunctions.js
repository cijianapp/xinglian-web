...
import { insertMedia, isImageUrl, wrapLink } from "./insertFunctions";
...
export const withMedia = editor => {
  ...
  editor.isVoid = element => {
    if (["image", "video"].includes(element.type)) return true;
      ...
      for (const file of files) {
        ...
        if (mime === "image") {
          reader.addEventListener("load", () => {
            ...
            insertMedia(editor, file, url, "image");
          });
  ...
  editor.deleteForward = () => {
    ...
    if (nextNode !== undefined) {
      if (["image", "video"].includes(nextNode[0].type)) {
  ...
  editor.deleteBackward = () => {
    ...
    if (nextNode !== undefined) {
      if (thisNode[0].children[0].text === "") {
        if (["image", "video"].includes(nextNode[0].type)) {
          if (["image", "video"].includes(thisNode[0].type)) {
            Transforms.removeNodes(editor);
  ...
  editor.insertBreak = () => {
    ...
    if (["image", "video"].includes(thisNode[0].type)) {
      ...
      let endPoint = Editor.end(editor, [
